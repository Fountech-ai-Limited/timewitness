...
summary="${GITHUB_STEP_SUMMARY:-/dev/stdout}"
{
    ...
    echo "names: a bound of 222.628 ms on 2026-09-25 from the public install check at v0.4, and a bound"
    echo "of 287.147 ms on 2026-09-14 from the same check at v0. Those two are not a series and are not"
    echo "a trend."
    ...
    echo "special hardware and about 1 ms for a clock updated once a minute, as primary reference"
    echo "clocks are, are quoted from public research. Neither has been measured by us and neither is"
    echo "ours. Both are read from David Mills' Executive Summary: Computer Network Time"
