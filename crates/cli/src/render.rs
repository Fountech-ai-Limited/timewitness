...
pub fn usage() -> String {
    ...
    out.push_str("  timewitness countersign <value> | --from <file>\n");
    out.push_str("      Read one half of a countersign exchange and say what it establishes.\n");
    out.push_str("      No network and no account, the same as verify. A verified signature\n");
    out.push_str("      says the party holding that key signed that statement about its own\n");
    out.push_str("      clock, and nothing about whether the clock was right.\n\n");
    out.push_str("      --from <file>       read the header value out of a file rather than\n");
    out.push_str("                          taking it on the command line\n\n");
