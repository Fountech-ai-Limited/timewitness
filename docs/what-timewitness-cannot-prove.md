...
**On Windows, continuous integration installs and removes the agent as a service and nothing more.**
It does that on Windows and macOS runners for every pull request that touches the agent, and runs
everything else on Linux. The Windows time service contends with any other discipliner, which
